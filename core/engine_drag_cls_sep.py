...
from utils.train_val import FocalLoss, CELoss
...
class Trainer():
    def __init__(self, cfg, device):
        ...
        # ===== Loss =====
        self.compute_loss = self.set_criterion(self.cfg, self.device)
        # ===== resume =====
    ...
    def train_one_epoch(self, epoch):
        ...
        for step, batch_data in pbar:
            ...
            onehot_t = batch_data[5].to(self.device)
            yt = torch.cat([y, t], dim=1).to(self.device)
            #
            imgs = batch_data[0].to(self.device)
            #
            out_drag, out_total = self.model(imgs, x.to(self.device))
            loss, loss_dict = self.calc_loss(out_drag, yt, out_total, onehot_t)
            ...
            #
            result = self.calc_acc(yt, out_drag)
            cnt_correct += result[0]
            cnt_total += result[1]
            result_total = self.calc_acc_total(out_total, onehot_t)
            cnt_correct_total += result_total[0]
            #
            reset_acc = self.print_verbose(epoch, step, mean_loss, loss_dict,
                                           cnt_correct / cnt_total, cnt_correct_total / cnt_total, pbar)
            if reset_acc:
                cnt_correct = 0.0
                cnt_total = 0.0
                cnt_correct_total = 0.0
    ...
    def calc_loss(self, out_drag, yt, out_total, onehot_t):
        loss = 0
        for loss_key in list(self.compute_loss.keys()):
            if loss_key == 'tarreg':
                loss += self.compute_loss[loss_key]._calc(yt, out_drag, reduction='sum')
            elif loss_key == 'focal' or loss_key == 'ce':
                loss += self.compute_loss[loss_key](out_total, onehot_t) * 10
        loss_dict = dict()
        with torch.no_grad():
            for loss_key in list(self.compute_loss.keys()):
                if loss_key == 'tarreg':
                    loss_dict.setdefault(loss_key, self.compute_loss[loss_key]._calc(yt, out_drag.detach(), reduction='sum'))
                elif loss_key == 'focal' or loss_key == 'ce':
                    loss_dict.setdefault(loss_key, self.compute_loss[loss_key](out_total.detach(), onehot_t) * 10)
        return loss, loss_dict

    @staticmethod
    def set_criterion(cfg, device):
        loss_names = cfg.loss.split('+')
        #
        loss_dict = dict()
        from models.dragonnet.loss import TarReg_Loss
        loss_dict.setdefault('tarreg', TarReg_Loss(alpha=3., beta=3.))
        #
        for loss_name in loss_names:
            if loss_name == 'focal':
                loss_dict.setdefault(loss_name, FocalLoss(cfg.lossredu, device))
            elif loss_name == 'ce':
                loss_dict.setdefault(loss_name, CELoss(cfg.lossredu)).to(device)
            elif loss_name == '':
                continue
            else:
    ...
    def validation(self, epoch, global_step):
        ...
        for step, batch_data in enumerate(self.val_loader):
            ...
            imgs = batch_data[0].to(self.device)
            #
            out_drag, out_total = model(imgs, x.to(self.device))
            #
            result = self.calc_acc(yt, out_drag)
            result_total = self.calc_acc_total(out_total, onehot_t)
            cnt_correct += result[0]
            cnt_total += yt.shape[0]
            cnt_correct_total += result_total[0]
            cnt_correct_total_total += onehot_t.shape[0]
        #
        write_tbacc(self.tblogger, acc=cnt_correct / cnt_total, step=(epoch * self.max_stepnum) + global_step,
                    task='val/dragon')
        write_tbacc(self.tblogger, acc=cnt_correct_total / cnt_total, step=(epoch * self.max_stepnum) + global_step,
                    task='val/total')
        #
        self.model.train()
    ...
    @staticmethod
    def calc_acc_total(pred, true):
        pred = F.softmax(pred, dim=1)
        pred[pred > 0.5] = 1
        pred[pred <= 0.5] = 0
        cnt_correct = (torch.argmax(pred, dim=1) == torch.argmax(true, dim=1)).sum()
        cnt_total = pred.shape[0]
        return cnt_correct.float().item(), float(cnt_total)
    ...
    def print_verbose(self, epoch, step, mean_loss, loss_dict, acc, acc_total, pbar):
        reset = False
        if (epoch != 0 or (epoch == 0 and step != 0)) and (
            ...
            for key in list(loss_dict.keys()):
                write_tbloss(self.tblogger, losses=loss_dict[key].item(), name=key, step=(epoch * self.max_stepnum) + step)
        if (epoch != 0 or (epoch == 0 and step != 0)) and (
                (epoch * self.max_stepnum + step) % self.cfg.training_params.acc_freq == 0):
            write_tbacc(self.tblogger, acc=acc, step=(epoch * self.max_stepnum) + step, task='train/dragon')
            write_tbacc(self.tblogger, acc=acc_total, step=(epoch * self.max_stepnum) + step, task='train/total')
            reset = True
        if (epoch != 0 or (epoch == 0 and step != 0)) and (
    ...
    def get_data_loader(self):
        object = lesionDataset(self.cfg, None, mode=self.cfg.mode, task='train')
        train_loader = DataLoader(
            object, batch_size=self.cfg.batchsize, shuffle=True,
            num_workers=self.cfg.workers, collate_fn=lesionDataset.collate_fn,
            pin_memory=True
        )
        setattr(self.cfg, 'num_patches', int(object.get_num_patches()))
        #
        object = lesionDataset(self.cfg, None, mode=self.cfg.mode, task='test')
    ...
    def get_model(cfg, device):
        from models.dragResnet.drag3d import drag3d as net
        model = net(cfg=cfg, num_patches=cfg.num_patches).to(device)
        return model
