...
def dist2bbox(distance, anchor_points, box_format='xyxy'):
    '''
        ...
        ---
        anchor_points: the center points of anchors
        distance: the predicted reg_dist_list, shape=(batch, w*h*num_layers, 4)
        ---
    '''
    # lt: distance from center to left-top
    # rb: distance from center to right-bottom
    # shape of lt and rb: (batch, , 2)
    lt, rb = torch.split(distance, 2, -1)
    # lt[0, 0, 0] is the distance between the center point of an anchor and left-top point along x-axis
    # lt[0, 0, 1] is the distance between the center point of an anchor and left-top point along y-axis
    # A: a lt point, B: a rb point, C: center of an anchor
    # Note that the box around C is a grid cell, and C is the center point of an anchor (and the center of the grid cell).
    # A is the left-top point and B is the right-bottom point.
    # The box created by A and B is a bbox of an object.
    # A = (Ax, Ay), B = (Bx, By), C = (Cx, Cy)
    # lt[0, 0, 0] = |Ax-Cx|, lt[0, 0, 1] = |Ay-Cx|
    # rb[0, 0, 0] = |Bx-bx|, rb[0, 0, 1] = |By-Cx|
    # (So... anchor free...? since there is no anchor size such as anchor's width and anchor's height)
    #                         A ----------- |
    #                         |             |
    #                         |             |
    #                         |    a----    |
    ...
    if box_format == 'xyxy':
        ...
        c_xy = (x1y1 + x2y2) / 2
        wh = x2y2 - x1y1
        # bbox: shape=(batch, w*h*num_layers, 4)
...
def check_version(current='0.0.0', minimum='0.0.0', name='version ', pinned=False, hard=False, verbose=False):
    ...
    if hard:
        info = f'?? {name}{minimum} is required by YOLOv6, but {name}{current} is currently installed'
