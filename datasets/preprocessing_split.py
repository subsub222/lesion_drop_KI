...
def split_train_val(path, train_val_ratio):
    ...
    df['day_name'] = df['수술연월일'].dt.day_name()
    df = df.drop(columns=['수술연월일','DCIS_or_LCIS_type'])

    numeric_cols = ['나이', '암의 장경', 'ER_Allred_score', 'PR_Allred_score', 'KI-67_LI_percent']
    ignore_cols = ['ID', 'img_path', 'mask_path', 'N_category']
    # minmax_cols = ['month', 'day'] #new preprocessing
    ...
    y_data = df.drop(
        ...
        columns=['나이', '진단명', '암의 위치', '암의 개수', 'ER',
                 ...
                 'HER2_IHC', 'N_category', 'NG', 'HG', 'HG_score_1',
                 'HG_score_2', 'HG_score_3', 'DCIS_or_LCIS_여부',
                 'T_category','year', 'month', 'day', 'day_name','HER2_SISH', 'HER2_SISH_ratio', 'BRCA_mutation']
    )
    y_keys = list(y_data.keys())
    y_data = np.array(y_data)
    #
    x_data = df.drop(
        columns=['암의 장경', 'N_category', 'KI-67_LI_percent']
    ...
    t_data = df.drop(columns=['나이', '진단명', '암의 위치', '암의 개수', '암의 장경', 'NG', 'HG', 'HG_score_1',
                              ...
                              'HER2_IHC', 'year', 'month', 'day', 'day_name','HER2_SISH', 'HER2_SISH_ratio', 'BRCA_mutation'])
    ...
    for i in tqdm(range(train_x_mask.shape[0]), total=len(train_x_mask), leave=True, ascii=True,
        ...
        id = x[0]
        img_path = os.path.join(base_path, 'ori_train_image', x[2].split('/')[-1:][0])
    ...
    with open(os.path.join(base_path, 'splitted_data.json'), 'w') as f:
