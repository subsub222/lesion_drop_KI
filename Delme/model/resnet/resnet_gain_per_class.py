...
class resnet(nn.Module):
    def __init__(self, cfg, net_name='resnet6', num_classes=5):
        ...
        self.conv5_x = nn.Sequential(
            self._make_layer(block, 512, num_block[3], 2) if num_block[3] != 0 else nn.Sequential(),
        )
    ...
    def forward(self, input, label, mask_label, gain=False):
        # Classification
        ...
        if gain:
            ...
            with torch.no_grad():
                # make masked input
                maskedimg, mask_map = self._make_masked_input(input, self.gcam)
                #
                loss_am = 0
                loss_map = 0
                for batch_idx in range(label.shape[0]):
                    ...
                    for cls_idx in range(len(temp_label)):
                        if temp_label[cls_idx] == 1:
                            output_am = self._forward_cl(maskedimg[cls_idx][batch_idx].unsqueeze(dim=0))
                            output_am_score_softmax = F.softmax(output_am, dim=1)
                            #
                            for kk in range(len(temp_label)):
                                if kk == cls_idx:
                                    loss_am += output_am_score_softmax[0, kk]
                                    loss_map += F.mse_loss(mask_label[batch_idx][0][kk].to(temp_label.device),
                                                           mask_map[cls_idx][batch_idx][0])
                                else:
                                    if temp_label[kk] == 1:
                                        loss_am += (1 - output_am_score_softmax[0, kk]) * self.am_neg_factor
            self.loss_am = loss_am / label.shape[0] * self.am_factor + loss_map * 2
        return out_cl
