...
def write_tbloss(tblogger, losses, step):
    tblogger.add_scalar("training/loss/total", losses[0], step + 1)
    tblogger.add_scalar("training/loss/regression", losses[1], step + 1)
    tblogger.add_scalar("training/loss/standard", losses[2], step + 1)
...
def write_tbacc(tblogger, acc, epoch, task):
    tblogger.add_scalar("acc/" + task, acc, epoch + 1)
...
def write_tbimg(tblogger, imgs, step, type='train'):
    ...
    if type == 'train':
        for idx, img in enumerate(imgs):
            tblogger.add_image(f'train_img_{idx + 1}', img, step + 1, dataformats='HWC')
    elif type == 'val':
        for idx, img in enumerate(imgs):
