...
from core.engine_drag_cls_sep import Trainer
from utils.config import Config
from cmd_in_drag_cls import get_args_parser
...
def check_and_init(args):
    ...
    for key, value in args._get_kwargs():
        if key == 'datajson':
            setattr(cfg.dataset, 'json_path', os.path.join(cfg.dataset.base_path, value + '.json'))
        elif key == 'inputsize':
            setattr(cfg.dataset, 'input_size', value)
        else:
...
def print_cfg(config):
    ...
    for key in list(config.keys()):
        if not isinstance(config._cfg_dict[key], dict):
            print("{0:>20} :".format(str(key)) + " {0:<20}".format('-' if config._cfg_dict[key] is None else config._cfg_dict[key]))
        else:
